...
namespace DynamicCompile
{
    public class BTree<TK, TP> where TK : IComparable<TK>
    {
        ...
        {
            int i = node.Entries.TakeWhile(entry => key.CompareTo(entry.Key) > 0).Count(); // will look for key
        ...
        public void Delete(TK keyToDelete)
        {
            ...
            //если последняя запись корня была перенесена в дочерний узел, удаляем ее 
        ...
        private void DeleteInternal(Node<TK, TP> node, TK keyToDelete)
        {
            ...
            // found key in node, to delete him from it
        ...
        /// <param name="keyToDelete">Key to be deleted.</param>
        /// <param name="keyIndexInNode">Index of key in the node.</param>
        private void DeleteKeyFromNode(Node<TK, TP> node, TK keyToDelete, int keyIndexInNode)
        {
            // if leaf, just remove it from the list of entries
            if (node.IsLeaf)
            {
        ...
        /// <summary>
        /// Helper method that deletes a predecessor key  for a given node.
        /// </summary>
